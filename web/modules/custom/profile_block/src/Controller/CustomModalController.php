...
class CustomModalController extends ControllerBase
{
  ...
  { 
    ...
    foreach ($fuid as $row) 
    {
      ...
      {
        $follow_link = Url::fromRoute('profile_block.unfollow',['user_id' => $fid]);
        ...
        foreach ($current_fuid as $row) 
        {
          ...
          {    
            ...
            $follow_link = Url::fromRoute('profile_block.follow',['user_id' => $fid]);
            $follow_link->setOptions([
              'attributes' => [
                'class' => ['use-ajax', 'button', 'button--small'],
                'data-dialog-type' => 'modal',
      ...
      $flag_link = Url::fromRoute('profile_block.unfollow',['user_id' => $fid]);
